...
void KippenKot::init(MyBluetooth *myBluetooth, Ina219Proxy *ina219, LightMeterProxy *lightMeter, ClockProxy *myclock, Door *door, Printer *printer)
{
    ...
    if (!PRD)
    { // do not initialiase Serial in PRD (no USB cable connected) as it hangs the esp
        Serial.begin(115200);
    }
...
void KippenKot::handleBleCommands()
{
    ...
    else if (c == DATE_AND_TIME_BL_COMMAND)
    {                // date and time
    ...
    else if (c == START_NIGHT_HOUR_BLE_COMMAND)
    {                // night start
        delay(1000); // wait for next message to arrive
    ...
    else if (c == CRUSHED_CHICKEN_CURRENT_COMMAND)
    {                // cut of power crushed chicken
        delay(1000); // wait for next message to arrive
    ...
    else if (c == AUTOMATIC_ENABLED_COMMAND)
    { // re-enable automatic handling
        _printer->print("\nAutomatic mode enabled again\n");
        automaticMode = true;
    }
    else if (c == IGNORE_LIGHT_SENSOR)
    { // disable light sensor
        _printer->print("\nLight sensor disabled\n");
        lightSensorEnabled = false;
    }
    else if (c == USE_LIGHT_SENSOR)
...
void KippenKot::closeDoorCompletely(int tryCount)
{
    ...
    while (millis() < stopTime && !stopReceived && !stopMaxCurrent)
    {
        ...
        if (millis() < (startTime + 5500) && stopMaxCurrent)
        { // always stop in the first 5.5 sec afterwards assume its the door not a chicken
